...
export default function PostcodeChecker({ 
  ...
  const handleCheck = (e: React.FormEvent) => {
    ...
    if (!coveredPostcodes.includes(postcodeArea)) {
    ...
    console.log('Checking postcode:', {
      original: postcode,
      normalized: normalizedPostcode,
      extracted: postcodeArea,
      isCovered: coveredPostcodes.includes(postcodeArea),
      allCovered: coveredPostcodes
    })
    ...
    setTimeout(() => {
      if (coveredPostcodes.includes(postcodeArea)) {
        ...
        setTimeout(() => {
          router.push(`/get-in-touch?postcode=${normalizedPostcode}&area=${postcodeAreas[postcodeArea] || ''}`)
        }, 1500)
      } else {
        setStatus('error')
        setTimeout(() => {
