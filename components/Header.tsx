        ...
        <div className="fixed inset-0 top-20 z-40 bg-black/95 backdrop-blur-lg lg:hidden">
          <div className="mx-auto max-w-md px-4 py-8">
            <div className="space-y-6">
              {/* Services */}
              <div className="rounded-2xl border border-white/15 bg-black/60 p-6">
                <h3 className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-white/60">
                  Services
                </h3>
                <div className="space-y-2">
                  {SERVICE_LINKS.map((service) => (
                    <Link
                      ...
                      className="block rounded-lg border border-white/10 bg-white/5 px-4 py-3 text-sm font-medium text-white/80 transition-all hover:border-brand-red/40 hover:bg-brand-red/10 hover:text-white"
                    >
                      {service.title}
                    </Link>
                  ))}
              ...
              {/* Other Nav Links */}
              <div className="space-y-2">
                {NAV_ITEMS.filter((item): item is NavLinkItem => item.type === 'link').map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    onClick={() => setOpen(false)}
